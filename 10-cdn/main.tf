resource "aws_cloudfront_distribution" "web_cdn" {
  origin {
    domain_name              =  "web-${var.environment}.${var.zone_name}" #web-dev.pspkdevops.online
  ...
  aliases = ["web-${var.common_tags.Component}.${var.zone_name}"]   #web-cdn.pspkdevops.online
...
## this restriction is optional section, if required we can use.
  restrictions {
    geo_restriction {
  ...
  viewer_certificate {
    ...
    ## SNI - Server Name Indication  --->> meaning https onlly
  }
}
...
module "records" {
  ...
  records = [
    {
      name    = "web-cdn" #web-cdn.pspkdevops.online
      type    = "A"
